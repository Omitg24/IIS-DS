...
public class TriangleTool implements Tool {
	...
	public void click(int x, int y) {
		...
		vertexCounter++;
		if (vertexCounter == 3) {
			Figure triangle = new Triangle(vertex[0], vertex[1], vertex[2]);
			editor.getDrawing().addFigure(triangle);
			editor.getHistory().addChange(new CreationChanges(editor.getDrawing(), triangle));;
			vertexCounter=0;
			editor.reloadTool();
		}
	}
